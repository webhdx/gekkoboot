...
_M2Loader_WriteByte(u8 addr, u8 data) {
	u32 dat = 0x8000'0000 | (addr << 24) | (data << 16);
	EXI_Lock(EXI_CHANNEL_0, EXI_DEVICE_2, NULL);
	EXI_Select(EXI_CHANNEL_0, EXI_DEVICE_2, EXI_SPEED32MHZ);
...
_M2Loader_WriteU16(u16 data) {
	...
	u32 dat = 0xD000'0000 | (((data >> 8) & 0xff) << 16) | ((data & 0xff) << 8);
	EXI_Lock(EXI_CHANNEL_0, EXI_DEVICE_2, NULL);
	EXI_Select(EXI_CHANNEL_0, EXI_DEVICE_2, EXI_SPEED32MHZ);
...
_M2Loader_ReadBuffer(u32 *dst) {
	...
	// (31:29) 011b | (28:24) 10000b | (23:16) <num_words_LSB> | (15:8) <num_words_MSB> | (7:0)
	// 00h (4 bytes)
	u32 dat = 0x7000'0000 | ((dwords & 0xff) << 16) | (((dwords >> 8) & 0xff) << 8);
	EXI_Lock(EXI_CHANNEL_0, EXI_DEVICE_2, NULL);
	EXI_Select(EXI_CHANNEL_0, EXI_DEVICE_2, EXI_SPEED32MHZ);
	EXI_ImmEx(EXI_CHANNEL_0, &dat, 4, EXI_WRITE);
...
_M2Loader_WriteBuffer(u32 *src) {
	...
	u32 dat = 0xF000'0000 | ((dwords & 0xff) << 16) | (((dwords >> 8) & 0xff) << 8);
	EXI_Lock(EXI_CHANNEL_0, EXI_DEVICE_2, NULL);
	EXI_Select(EXI_CHANNEL_0, EXI_DEVICE_2, EXI_SPEED32MHZ);
	EXI_ImmEx(EXI_CHANNEL_0, &dat, 3, EXI_WRITE);
	...
	dat = 0;
	EXI_ImmEx(
		EXI_CHANNEL_0, &dat, 1, EXI_WRITE
	); // Burn an extra cycle for the M.2 Loader to know to stop serving data
	EXI_Deselect(EXI_CHANNEL_0);
	EXI_Unlock(EXI_CHANNEL_0);
}
...
_M2Loader_DriveIdentify() {
	...
	do {
		...
		usleep(100'000); // sleep for 0.1 seconds
		retries--;
#ifdef _M2LDR_DEBUG
	...
	if (M2LoaderDriveInfo.lba48Support) {
		...
		M2LoaderDriveInfo.sizeInSectors =
			(u64) (((u64) lbaHi << 32) | (lbaMid << 16) | lbaLo);
		M2LoaderDriveInfo.sizeInGigaBytes =
			(u32) ((M2LoaderDriveInfo.sizeInSectors << 9) / 1024 / 1024 / 1024);
	} else {
		...
		M2LoaderDriveInfo.sizeInSectors =
			((*(u16 *) &buffer[ATA_IDENT_LBASECTORS + 1]) << 16)
...
M2Loader_Unlock(int useMaster, char *password, int command) {
	...
	if (!retries) {
#ifdef _M2LDR_DEBUG
		printf("UNLOCK (%08X) Drive did not respond in time, failing M.2 Loader "
		       "init..\r\n",
		       tmp);
#endif
...
_M2Loader_ReadSector(u64 lba, u32 *Buffer) {
	...
	_M2Loader_WriteByte(
		ATA_REG_COMMAND,
		M2LoaderDriveInfo.lba48Support ? ATA_CMD_READSECTEXT : ATA_CMD_READSECT
	);
...
_M2Loader_WriteSector(u64 lba, u32 *Buffer) {
	...
	_M2Loader_WriteByte(
		ATA_REG_COMMAND,
		M2LoaderDriveInfo.lba48Support ? ATA_CMD_WRITESECTEXT : ATA_CMD_WRITESECT
	);
...
static bool
__m2ldr_startup(void) {
	return M2Loader_IsDriveInserted();
}

static bool
__m2ldr_isInserted(void) {
	return M2Loader_IsDriveInserted();
}

static bool
__m2ldr_readSectors(sec_t sector, sec_t numSectors, void *buffer) {
	return !_M2Loader_ReadSectors((u64) sector, numSectors, buffer);
}

static bool
__m2ldr_writeSectors(sec_t sector, sec_t numSectors, void *buffer) {
	return !_M2Loader_WriteSectors((u64) sector, numSectors, buffer);
}

static bool
__m2ldr_clearStatus(void) {
	return true;
}

static bool
__m2ldr_shutdown(void) {
	return true;
}
